...
public static class LuckshotMath
{
	...
	// RIGIDBODY
	public static Vector3 RigidbodyTransformPoint(this Rigidbody rigidbody, Vector3 localPos)
	{
		Vector4 homogenousPoint = new Vector4(localPos.x, localPos.y, localPos.z, 1.0f);
		Vector4 transPoint = Matrix4x4.TRS(rigidbody.position, rigidbody.rotation, rigidbody.transform.lossyScale) * homogenousPoint;

		Vector3 pos = new Vector3(transPoint.x, transPoint.y, transPoint.z) / transPoint.w;
		return pos;
	}

	public static Vector3 RigidbodyTransformDirection(this Rigidbody rigidbody, Vector3 localDir)
	{ return Matrix4x4.Rotate(rigidbody.rotation) * localDir; }

	public static Vector3 RigidbodyTransformScale(this Rigidbody rigidbody, Vector3 localVector)
	{ return Matrix4x4.TRS(Vector3.zero, rigidbody.rotation, rigidbody.transform.lossyScale) * localVector; }
	...
	public static bool TryGetComponentInChildren<T>(this GameObject go, out T t, bool includeInactive = false) where T : Component
	{
		t = go.GetComponentInChildren<T>(includeInactive);
		return t != null;
	}

	public static bool TryGetComponentInChildren<T>(this Component component, out T t, bool includeInactive = false) where T : Component
	{return component.gameObject.TryGetComponentInChildren(out t, includeInactive); }

	public static bool TryFindRecursive(this Transform transform, string name, out Transform child)
	{
		child = FindRecursive(transform, name);
		return child != null;
	}
